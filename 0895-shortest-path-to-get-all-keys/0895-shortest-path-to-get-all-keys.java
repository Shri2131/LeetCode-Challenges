class Solution {
    public int shortestPathAllKeys(String[] grid) {
        ...
        Queue<int[]> queueSol = new LinkedList<>();
        int[][] movesTaken = new int[][] {{0, 1}, {1, 0}, {-1, 0}, {0, -1}};
        ...
        queueSol.offer(new int[] {startR, startC, 0, 0});
        seen.put(0, new HashSet<>());
        seen.get(0).add(new Pair<>(startR, startC));    
        
        while (!queueSol.isEmpty()) {
            int[] cur = queueSol.poll();
            int curR = cur[0], curC = cur[1], keys = cur[2], dist = cur[3];
            for (int[] move : movesTaken) {
                ...
                if (newR >= 0 && newR < m && newC >= 0 && newC < n && grid[newR].charAt(newC) != '#') {
                    ...
                    if (keySet.contains(cell)) {
                        ...
                        // Otherwise, just add this state to seen and queueSol.
                        if (newKeys == allKeys) {
                            return dist + 1;
                        }
                        seen.putIfAbsent(newKeys, new HashSet<>());
                        seen.get(newKeys).add(new Pair<>(newR, newC));
                        queueSol.offer(new int[] {newR, newC, newKeys, dist + 1});
                    }
                    ...
                    // We can walk to this cell if we haven't been here before with the same key state.
                        ...
                        queueSol.offer(new int[] {newR, newC, keys, dist + 1});
                    }
                }
            }
        }
